function [x0_scaled, rel_errors] = fit_positive_region(data_V, data_JD, x0_scaled, params, config, options_lm, prev_errors, optcfg)
    ...
    if ~isempty(low_pos_idx)
        ...
        low_pos_errFun = @(x_opt) errorFunctionPartialWeighted(x_opt, x0_scaled, param_mask, low_pos_V, low_pos_JD, params, config, config.regularization.prior, low_pos_prev);
    ...
    if ~isempty(high_pos_idx)
        ...
        high_pos_errFun = @(x_opt) errorFunctionPartialWeighted(x_opt, x0_scaled, param_mask, high_pos_V, high_pos_JD, params, config, config.regularization.prior, high_pos_prev);
    ...
    if ~isempty(pos_idx)
        ...
        pos_errFun = @(x_opt) errorFunctionPartialWeighted(x_opt, x0_scaled, param_mask, pos_V, pos_JD, params, config, config.regularization.prior, pos_prev);
