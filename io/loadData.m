function [data_V, data_JD, V] = loadData(varargin)
%LOADDATA 选择文件并读取电流数据
%   [V, JD] = LOADDATA() 弹出对话框让用户选择包含电流密度的文件，
%   然后询问需要读取的单元格范围(如 A1:A81)。电压向量默认为
%   -0.5V 至 0.3V，步长 0.01V。也可以通过输入 (v_start,v_step,v_end)
%   或一个电压向量 V 来指定自定义范围。如果文件选择被取消或
%   读取失败，将返回内置示例数据。

    % 解析电压范围参数
    defaultV = (-0.5:0.01:0.3)';
    if nargin == 1 && isnumeric(varargin{1}) && numel(varargin{1}) > 1
        V = varargin{1}(:);            % 用户直接传入电压向量
    elseif nargin == 3
